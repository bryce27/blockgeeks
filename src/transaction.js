...
function addPadding(num, bytes) {
    // Each hex is 4 bits, so multiply bytes by 2 to get number of hex digits
    while (num.length < bytes*2) num = "0" + num;
    return num;
}

function toLE(input) {
    // Split string into array of 2 chars each
    // then reverse and join back to string
    return input.match(/.{2}/g).reverse().join('');
}

function getFee() {
    return "10000"; // fixed for now
}

function seralizeObjVal(obj){
    // ignore all keys and concat just the values
